...
function __string_id(id){
	return slice(string(id), 14)
}
...
else return __variable_string__(name, variable_string_info)
}
...
static next_pos = function(str, startpos, has_accessor){
	...
	var pos1 = string_pos_ext(".", str, startpos+1)
	var pos2 = has_accessor ? string_pos_ext("[", str, startpos+1) : 0
...
if not is_string(name)
{
	if is_method(name) or (is_numeric(name) and better_script_exists(name)) switch scr
	{
		...
		case variable_string_info:	 return {value: undefined, scope: undefined, exists: true, error: undefined, plain: name, simple: true}
	}
...
if string_char_at(name, 1) == "." name = is_struct(self) ? string_delete(name, 1, 1) : (__string_id(id)+name)
...
var ds_scope = (is_numeric(scope) and instance_exists(scope)) ? __string_id(scope.id) : ""
